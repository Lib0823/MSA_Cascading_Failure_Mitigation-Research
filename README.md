...
본 연구의 주장은 이것이다 — **연쇄 장애는 자원 증설만으로 대응할 수 없으며, 선제 대응 특유의 두 제약 하에서 신뢰도에 따라 이질적 조치를 선택하는 정책이 신뢰도를 무시하고 고르는 정책보다 우월하다.**

선제 대응과 신뢰도 기반 조치 선택은 각각 단독으로는 선행연구가 다룬다(전자는 GRAF·DeepScaler·AGQ가 **단일 조치**로, 후자는 Safe Remediation이 **사후 복구**로). 그러나 **결합하면 사후 복구에는 없는 두 제약이 생긴다**:

- **(a) 장애 미발생 시 개입 비용** — 사후에는 장애가 실재하므로 *"장애가 없는데 개입"* 이라는 경우 자체가 없다. 선제에서는 예측이 빗나가면 멀쩡한 시스템에 조치를 가한 순손실이 남는다. (원인은 예측 불확실성이 진단 불확실성보다 크다는 것이다)
- **(b) 조치 리드타임 제약** — `ℓₐ ≥ L_pred` 인 조치는 후보에서 자동 배제된다. 사후에도 실행시간은 걸리지만 *"늦어서 그 조치를 못 쓴다"* 는 **선택 제약으로 작동하지 않는다.**

본 연구는 이 두 제약 하에서 조치별 임계값 `θₐ`를 유도하는 Policy Engine을 제안하고, **두 제약을 모두 실측**해 그 필요성을 실증한다.

## Motivation

- 모놀리식 → K8s 분산 전환 시 연쇄 장애 위험 급증
- **자원 증설이 답이 아닌 두 가지 경우가 실재한다**:
  - **역효과** — 앱이 HPA로 스케일아웃되면 **총 DB 커넥션 요구 = 인스턴스 수 × 풀 크기** 가 늘어 DB `max_connections`를 초과한다
...
| # | 시나리오 | 정답 조치 | 검증 대상 |
|---|---|---|---|
| S1 | **DB 커넥션 고갈** (앱 HPA → 총 커넥션 요구가 DB `max_connections` 초과) | **Bulkhead** | 증설이 **역효과** |
