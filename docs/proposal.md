...
**Safety Guard(저신뢰 보류)**: p가 모든 θₐ에 못 미치면(= 앙상블 분산이 커 저신뢰) 어떤 조치도 보류를 이기지 못해 자동으로 보류된다. 즉 "분산↑ → 저신뢰 → 극단 조치 유보"라는 Safety Guard가 외부 하드코딩 규칙이 아니라 비용함수에서 자연히 유도된다. 이 보류의 타당성(저신뢰일 때 실제로 조치를 참는 게 이득인지)은 오탐/Drop Rate로 검증한다(§4-4). p 자체의 신뢰성은 보정(ECE)으로 별도 검증한다(§4-2·§4-4).
