...
4. for each v∈V:
       ...
       if cand ≠ ∅:
           ...
           emit(대상=v, 조치=a*, TTL=lease)                    # escalate-only, 시한부
       else:
...
(주의: 본 연구 벤치마크는 고정 11노드 위상이라 위상 변경은 실험 과제가 아니라 프로덕션 확장 시 대응 논거다.)
